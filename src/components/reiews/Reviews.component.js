import React from 'react'

const Reviews = () => {
    return (
        <div className="reviews">
            <div className="title-bar">
                <h1 className="title">Reviews</h1>
                <h4 className="pagination">1/10 &larr; &rarr;</h4>
            </div>
            <div className="reviews-cards">
                <div className="card">
                    <div className="person-info">
                        <div className="person-heading">
                            <img src="data:image/jpeg;base64,"
                                width="50px"
                                height="50px"
                                alt="" />
                            <div className="name-country">
                                <h1>Hellen Jummu</h1>
                                <div className="country">
                                    <img src="http://namibiadailynews.info/wp-content/uploads/2020/01/USA-512.png" width="25px" height="auto" alt="" />
                                    <p>PALO ALTO, CA</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <p className='person-para'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vitae in donec in nisi vitae. Vestibulum pellentesque eget laoreet adipiscing.</p>
                </div>
                <div className="card">
                    <div className="person-info">
                        <div className="person-heading">
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSILg48X1W2TnwhKbimLBfOmmjlFNARCJwDKQ&usqp=CAU"
                                width="50px"
                                height="50px"
                                alt="" />
                            <div className="name-country">
                                <h1>Isaac Oluwatemilorun</h1>
                                <div className="country">
                                    <img src="http://namibiadailynews.info/wp-content/uploads/2020/01/USA-512.png" width="25px" height="auto" alt="" />                                    <p>PALO ALTO, CA</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <p className='person-para'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vitae in donec in nisi vitae. Vestibulum pellentesque eget laoreet adipiscing.</p>                </div>
                <div className="card">
                    <div className="person-info">
                        <div className="person-heading">
                            <img src="https://cdna.artstation.com/p/assets/images/images/021/115/556/large/kushal-kush-hex-color.jpg?1570458842"
                                width="50px"
                                height="50px"
                                alt="" />
                            <div className="name-country">
                                <h1>Hellen Jummu</h1>
                                <div className="country">
                                    <img src="http://namibiadailynews.info/wp-content/uploads/2020/01/USA-512.png" width="25px" height="auto" alt="" />
                                    <p>PALO ALTO, CA</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <p className='person-para'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vitae in donec in nisi vitae. Vestibulum pellentesque eget laoreet adipiscing.</p>                </div>
            </div>
        </div>
    )
}

export default Reviews;
